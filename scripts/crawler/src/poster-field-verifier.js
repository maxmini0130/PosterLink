...
function normalizeResult(result) {
  ...
  return {
    ...
    correctedOrgName: result.correctedOrgName ? String(result.correctedOrgName).slice(0, 200) : null,
    confidence,
...
export async function verifyPosterFields(context = {}) {
  ...
  try {
    const prompt = [
      ...
      "Check whether the extracted deadline actually matches the application/event deadline mentioned in the text, and whether the extracted organization name matches the actual hosting/organizing body.",
      ...
      "If the extracted organization name is wrong, provide the correct one, otherwise null.",
      "Only propose a correction when the text gives clear, unambiguous evidence — do not guess.",
      "Field context:",
      buildFieldContext(context),
      "Return JSON only with: deadlineMatches boolean, correctedDeadline string|null (YYYY-MM-DD), orgNameMatches boolean, correctedOrgName string|null, confidence number 0..1, decision string, reason string.",
    ].join("\n");
...
export function applyFieldVerification(post, verification, { minConfidence = MIN_CONFIDENCE } = {}) {
  ...
  const shouldUseCorrectedOrgName =
    verification.correctedOrgName && verification.orgNameMatches === false;

  return {
    deadline: shouldUseCorrectedDeadline ? verification.correctedDeadline : (post.deadline ?? null),
    orgName: shouldUseCorrectedOrgName ? verification.correctedOrgName : (post.site ?? null),
  };
}
