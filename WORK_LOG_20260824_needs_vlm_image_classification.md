...
## Batch 12 is_real_poster evidence apply

Applied after explicit user approval:

- Approved phrase:
  `VLM batch12 20건 결과를 is_real_poster evidence로 운영 DB 적용 승인합니다.`
- Applied rows: 20
- Field: `is_real_poster`
- Extractor: `poster-detection-signals-v1`
- Value: `true` x 19, `false` x 1
...
Post-apply DB verification confirmed 20 rows for the batch12 poster ids.
...
- B: 3
- C: 376
- `critical_missing_is_real_poster`: 52
- `critical_missing_deadline_type`: 261
- `critical_low_confidence_is_real_poster`: 4
- calendar/deadlineAlert gate: 115
...
## Batch 14 production apply

Applied after explicit user approval:

- Approved phrase:
  `needs-vlm image classification batch14 20 rows operating DB apply approved.`
- Original approval:
  `needs-vlm 이미지 분류 batch14 20건 운영 DB 적용 승인합니다.`
- Command:
  `pnpm --filter posterlink-crawler image:backfill -- --limit=20 --concurrency=1 "--statuses=published,review" --needs-vlm-only --apply --output=data/results/needs-vlm-image-classification-batch14-apply.json`
- Candidate count: 20
- Applied count: 20
- Failed count: 0
- Non-poster count: 0
...
- Checked rows: 538
- Evidence rows: 526
- `is_real_poster=true`: 514
- `is_real_poster=false`: 12
- Ambiguous: 12
- `needs_vlm`: 12

Batch14 evidence candidate check:

- Batch ids: 20
- Evidence rows: 20
- `is_real_poster=true`: 20

No batch14 `is_real_poster` evidence rows were applied in this step. That
conversion requires a separate operating DB approval.

## Batch 14 is_real_poster evidence apply

Applied after explicit user approval:

- Approved phrase:
  `VLM batch14 20 rows result to is_real_poster evidence operating DB apply approved.`
- Original approval:
  `VLM batch14 20건 결과를 is_real_poster evidence로 운영 DB 적용 승인합니다.`
- Applied rows: 20
- Field: `is_real_poster`
- Extractor: `poster-detection-signals-v1`
- Value: `true` x 20
...
Post-apply DB verification confirmed 20 rows for the batch14 poster ids.
...
- B: 3
- C: 369
- `critical_missing_is_real_poster`: 12
- `critical_missing_deadline_type`: 261
- `critical_low_confidence_is_real_poster`: 4
- calendar/deadlineAlert gate: 115
