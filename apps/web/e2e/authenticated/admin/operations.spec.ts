...
test.describe("관리자 운영 화면", () => {
  test("공지 발송 화면과 최근 공지 API를 확인한다", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "관리자 알림" })).toBeVisible({ timeout: 30_000 });
  ...
  test("기준정보 관리는 카테고리와 지역만 노출한다", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Master Data/ })).toBeVisible({ timeout: 30_000 });
