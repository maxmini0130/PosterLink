...
export default function AdminCrawlerPage() {
  ...
  const cards = useMemo(() => {
    ...
    return [
      ...
      { label: "검수 대기", value: stats?.review ?? 0, icon: <SearchCheck size={22} />, color: "bg-indigo-50 text-indigo-600" },
      { label: "게시 완료", value: stats?.published ?? 0, icon: <CheckCircle2 size={22} />, color: "bg-emerald-50 text-emerald-600" },
    ...
    <div className="pb-16">
      ...
      <div className="mb-8 grid gap-4 lg:grid-cols-[1fr_2fr]">
        <section className="rounded-[2rem] border border-gray-100 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <h2 className="mb-4 text-sm font-black text-gray-900 dark:text-white">소스별 최근 적재</h2>
          {loading ? (
        ...
        <section className="rounded-[2rem] border border-gray-100 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {summary.recent.map((poster) => (
                <div key={poster.id} className="flex gap-4 rounded-2xl border border-gray-100 p-4 dark:border-slate-800">
