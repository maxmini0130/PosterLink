...
- ✅ **2026-07-27 완료: 신용보증재단 17개의 실제 공고 게시판·SNS 계정.**
  ...
  반영함. 예외 2건만 남음: **광주**(인트로 페이지가 쿠키 기반으로 추정되는
  구조라 자동화로 진입 불가 — 홈페이지만 등록, 사람이 직접 방문 필요),
  **전남**(SNS 계정은 확인했으나 게시판이 JS 탭 전환 방식이라 안정적인
  목록 URL을 못 찾음). 부수 발견: 대구 재단의 시드 당시 도메인(`ttg.co.kr`)이
  실제로는 `dgsinbo.or.kr`로 리다이렉트되는 구도메인이었어서 정식 도메인으로
  교체함.
- ✅ **2026-07-27 완료: 17개 광역시도 경제진흥원/테크노파크 1차 등록.**
  `20260727030000_seed_metro_economic_promotion_agencies.sql`로 서울(SBA)·
  경기(GBSA)·부산(BEPA)·대구(DGTP)·인천(ITP)·광주(GEPA)·대전(DJTP)·
  울산(UEPA)·세종(SJTP)·강원(GWTP)·충북(CBTP)·충남(CEPA)·전북(JBBA)·
  전남(JEPA)·경북(GEPA)·경남(GNTP)·제주(JBA) 17곳을 등록함. **신용보증재단과
  달리 이번엔 브라우저 개별 방문 없이 웹검색만으로 1차 등록** — beachhead
  우선순위상 신용보증재단보다 낮은 항목이라 가벼운 방식을 택함. 그래서
  절반가량은 게시판 URL 신뢰도가 낮음(`reliability='low'`, notes에 명시) —
  실제 방문 확인은 후속 작업으로 남김. 기초 지자체(226개 시군구) 경제·
  일자리 부서는 이번에도 손 안 댐 — beachhead 우선순위상 가장 나중이어도
  될 항목으로 계속 유지.
