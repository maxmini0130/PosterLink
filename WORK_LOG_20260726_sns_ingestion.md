...
### ✅ 8-1. OPENAI_API_KEY 해결됨 (2026-07-27)
사용자가 키를 받아서 `scripts/crawler/.env.local`에 저장했고(첫 시도는 실수로
`.env.example`에 저장돼서 커밋되기 전에 바로잡음), 처음엔 그 프로젝트의 크레딧이
$0이라 `insufficient_quota` 오류가 났다가, 크레딧을 충전한 뒤 정상 동작 확인.

**검증 결과**: 실제 데이터 30건 + 폐기 합성 샘플 5건, 총 35건을 사람이 직접
읽고 비교. 공고 5/5, 소식 26~28/28(2건은 애매하지만 합리적인 판단), 폐기 5/5
정확 — Phase 2 완료기준 충족.

**밀려있던 재분류도 실행 완료**:
- "소식" 127건 재분류 → 공고 13건(진짜 공고였음, 승격), 소식 108건(유지),
  폐기 6건(전통시장 상점 홍보글 — `candidate_status='dismissed'`로 처리,
  raw는 보존). 오류 0건.
- 기존 발행 포스터 449건 카테고리 재분류는 아래 `WORK_LOG_20260727_site_quality.md`
  참고(같은 날 진행, 카테고리 통합 작업의 일부).
...
- ~~신용보증재단 17개 시도 지역재단 목록/홈페이지~~ → 완료(koreg.or.kr
  공식 페이지에서 이름-링크 정확히 매칭, 17개 전부 생존 확인).
- ✅ **2026-07-27 완료: 신용보증재단 17개 전부의 실제 공고 게시판·SNS 계정.**
  브라우저로 17곳 전부 직접 방문해서 실제 공지사항/사업공고 게시판 목록
  URL과 네이버블로그/페이스북/인스타그램 계정을 확인하고
  `20260727020000_update_credit_guarantee_foundation_boards_sns.sql`로
  반영함. 처음엔 광주·전남 2곳이 자동화로 안 뚫려서 남겨뒀었는데, 같은 날
  다시 시도해서 마저 해결함(`20260727050000`): **광주**는 인트로 페이지의
  "홈페이지 바로가기" 링크가 URL 직접 이동으로는 계속 인트로로 되돌아왔지만
  실제 클릭 이벤트로는 세션 쿠키가 생기며 정상 진입됨. **전남**은
  "재단소개" 메뉴 라벨의 실제 목적지가 공지사항 게시판이었음(라벨과 목적지가
  달랐던 것 — 이전엔 JS 탭 쪽만 보고 있어서 놓쳤음). 부수 발견: 대구 재단의
  시드 당시 도메인(`ttg.co.kr`)이 실제로는 `dgsinbo.or.kr`로 리다이렉트되는
  구도메인이었어서 정식 도메인으로 교체함. **17개 전부 완료.**
- ✅ **2026-07-27 완료: 17개 광역시도 경제진흥원/테크노파크 등록 + 방문 검증.**
  처음엔(`20260727030000`) 웹검색만으로 1차 등록했다가(신용보증재단보다
  우선순위 낮은 항목이라 가벼운 방식 선택), 같은 날 신용보증재단과 동일한
  수준으로 17곳 전부 브라우저 방문 검증까지 마침(`20260727060000`,
  `reliability='low'/'medium'` → `'high'`). 검증 중 잘못된 기관을 2건
  발견해서 교정: **강원**은 처음 등록한 `technopark.kr`가 강원 전용이
  아니라 "한국테크노파크진흥회"(전국 테크노파크 연합회) 사이트였음 —
  실제 강원테크노파크(`gwtp.or.kr`)로 교체. **울산**은 `uepa.or.kr`가
  실제로는 `ubpi.or.kr`(울산경제일자리진흥원)로 리다이렉트됨 — UEPA에서
  UBPI로 기관명이 개편된 것으로 보여 정식 도메인으로 교체. **광주**
  (`gepa.or.kr`)만 이번에도 검증 못 함 — SSL 인증서가 `jni.re.kr` 앞으로
  발급돼 있어(호스트명 불일치) 브라우저가 접속을 거부함, 도메인 자체가
  더 이상 유효하지 않을 가능성이 있어 다음에 새 도메인을 다시 찾아야 함.
  기초 지자체(226개 시군구) 경제·일자리 부서는 이번에도 손 안 댐 —
  beachhead 우선순위상 가장 나중이어도 될 항목으로 계속 유지.
